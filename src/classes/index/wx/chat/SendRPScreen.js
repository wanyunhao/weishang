...
export default class SendRPScreen extends Component {
    ...
    componentDidMount() {
        this.df_user_info = this.props.route.params.df_user_info;
            ...
            <View style={styles.container}>
                <WXNavigationBar title={this.state.type == 2 ? '转账' : '发红包'} rightText='发送' clickRText={()=>{
                    ...
                    let obj = {
                        id: getNow(),
                        c_id: this.c_id,//会话id
                        send_id : this.state.select == 2 ? RNStorage.user_id: this.df_user_info.id,
                ...
                <View>
                    <TitleAndSubCell isEdit={true} title={this.state.type == 2? '转账金额':'红包金额'} sub_title='请输入' keyboardType="numeric" value={this.state.hongbaoMoney} onChangeText={(text)=>{
                        this.setState({
                            hongbaoMoney:text
                        })
                    }}/>
                    <TitleAndSubCell isEdit={true} title={this.state.type == 2? '转账信息':'红包信息'} sub_title={this.state.type == 2 ?'请输入':'恭喜发财,大吉大利'} value={this.state.hongbaoText} onChangeText={(text)=>{
                        this.setState({
                            hongbaoText:text
                        })
                    }}/>
                ...
                <View style={{paddingHorizontal:15,backgroundColor: Colors.white}}>
                    ...
                    <View style={{flexDirection:'row',justifyContent:'space-between'}}>
                        <View style={{flexDirection:'row',alignItems:'center',paddingVertical:10,}}>
                            <View>
                                <XImage icon={isEmpty(this.state.df_user_info)?null:this.state.df_user_info.avatar} iconSize={35}/>
                                <Text style={{fontSize:12,marginTop:3}}>{isEmpty(this.state.df_user_info)?'':this.state.df_user_info.user_name}</Text>
                            </View>
